...
class Puddle:
    ...
    def inside(self, pose):
        return all([ self.lowerleft[i] < pose[i] < self.upperright[i] for i in [0, 1]]) # リストの中が全部trueだったらtrueを返す
...
class PuddleWorld(World):
    ...
    def append(self, obj):
        """puddleやrobotのオブジェクトは別途リストに追加したい
        """
...
class PuddleIgnoreAgent(EstimationAgent):
    ...
    def policy(cls, pose, goal):
        ...
        direction = (direction + 360*1000 + 180)%360 - 180 # 方角を-180~180に正規化
    ...
    def decision(self, observation=None):
        """motion_update, observation_updateをして、policyに従って制御情報生成
        """
        if self.in_goal:
            return 0.0, 0.0
...
def trial_puddle_world():
    ...
    a = PuddleIgnoreAgent(time_interval, kf, g, kf)
