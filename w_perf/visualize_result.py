...
def analyze(in_file, out_comp_file, out_summary_file, imp_rate_file):
    ...
    imp_rate_df = pd.DataFrame(columns=["agent", "improve", "trav_imp", "req_imp", "same", "degrade"])
    for agent in ["pomdp", "pomdp_wo_perf"]:
        improve = 0
        trav_imp = 0
        req_imp = 0
        same = 0
        ...
        buf_df = pd.DataFrame([[agent, improve/total, trav_imp/total, req_imp/total, same/total, (total-improve-trav_imp-req_imp-same)/total]], columns=imp_rate_df.columns)
...
if __name__ == "__main__":
    ...
    # simulation(initial_states, "./", param_list, "result_high.csv")

    ####################################
    # comparison
    ####################################
    # analyze("result_high.csv", "result_comparison_high.csv", "result_summary_high.csv", "result_summary_rate_high.csv")
