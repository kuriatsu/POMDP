...
class MDP:
    ...
    def action_value(self, action, index):
        ...
        for prob, index_after in self.state_transition(action, index):
            ...
            action_value = -1*efficiency -50*ambiguity -10*bad_int_request -1*int_request_penalty -1*self.delta_t + self.goal_value*self.final_state(index_after)
...
def trial_until_sat():
    ...
    try:
        ...
        with open("policy_int_req_penal.pkl", "wb") as f:
            pickle.dump(dp.policy, f)

        with open("value_int_req_penal.pkl", "wb") as f:
