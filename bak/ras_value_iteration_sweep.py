...
class MDP:
    ...
    def action_value(self, action, index):
        ...
        for prob, index_after in self.state_transition(action, index):
            ...
            comfort = abs(self.index_value(index_after, self.ego_state_index+1) - ego_speed)/self.delta_t > self.ordinary_G*9.8 

            mean_speed = (self.ideal_speed-self.min_speed)*0.5
            efficiency = (mean_speed - abs(ego_speed - mean_speed))/mean_speed
            # efficiency = abs(self.index_value(index_after, self.ego_state_index+1) - self.index_value(index, self.ego_state_index+1))/self.delta_t > 0.0 
            ...
            action_value = self.p_ambiguity*ambiguity \
